...
pub(crate) fn validate_deposit(
    ...
    let (btc_pool, rune_pool) = pool_utxo
        .as_ref()
        .map(|u| (u.sats, u.coins.value_of(&CoinId::rune(72798, 1058))))
...
pub(crate) fn validate_borrow(
    ...
    let (btc_output, rune_output) = (
        ...
        prev_utxo
            .coins
            .value_of(&CoinId::rune(72798, 1058))
...
impl StateView for PoolState {
    fn inspect_state(&self) -> StateInfo {
        StateInfo {
            txid: self.id.unwrap_or(Txid::zero()),
            nonce: self.nonce,
            btc_reserved: self.btc_supply(),
            coin_reserved: vec![CoinBalance {
                id: CoinId::rune(72798, 1058),
                value: self.rune_supply(CoinId::rune(72798, 1058)) as u128,
            }],
